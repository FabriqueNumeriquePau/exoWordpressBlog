			...
			if ( $recipe_ids ) {
				...
				foreach ( $unique_recipes as $unique_recipe ) {
					if ( WPRM_POST_TYPE === get_post_type( $unique_recipe['id'] ) ) {
						...
						$output['html'] .= '<div id="wprm-print-recipe-' . ( count( $recipes ) - 1 ) . '" data-recipe-id="' . $unique_recipe['id'] . '" class="wprm-print-recipe wprm-print-recipe-' . $unique_recipe['id'] . '" data-servings="' . esc_attr( $recipe->servings() ) . '">' . WPRM_Template_Manager::get_template( $recipe, 'print' ) . '</div>';
					}
				}
			}
